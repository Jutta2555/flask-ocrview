...
if len(sys.argv)==1 :
    pwdpicocr='/vsocr/ghbchkocr' #' folder for tif scan in txtfilealgorthm
else:
...
def txtlinetostruct(OcrData={},fileimg='',filetxt=''):
    ...
    if len(dum)==2:
        ...
        if len(re.findall(r"(\d)",dum[1]))>0 and len(re.findall(r"(\d)",dum[2]))>0 :
            person[KEYSDICT[9]]=dum[0] 
            person[KEYSDICT[10]]=dum[1]+dum[2] 
            person[KEYSDICT[22]]=dum[len(dum)-1]
        else:            
    ...
    if len(dum)==3:
        ...
        if len(re.findall(r"(\d)",dum[1]))>0 and len(re.findall(r"(\d)",dum[2]))>0 :
            person[KEYSDICT[11]]=dum[0] 
            person[KEYSDICT[12]]=dum[1]+dum[2] 
            person[KEYSDICT[23]]=dum[len(dum)-1]
        else:            
...
def main():
    ...
    for entry in dir_entries: # OCR_filev2() returns
        if entry.is_file() and  (".tif" in entry.name ): #or ".jpg" in entry.name  
            ...
            listoffiles=['addrocrg1_00000116.tif','addrocrg1_00000013-5.tif' ] 
            #เลขที่บ้าน หาย
